/* ================================================================
   ...
   APIs gratuites utilisées (aucune clé requise) :
     • Base Adresse Nationale (api-adresse.data.gouv.fr) — géocodage
       principal, très précis sur les numéros de rue en France
     • Nominatim (OpenStreetMap) — géocodage complémentaire (aéroports,
       Monaco, étranger...)
     • OSRM (Project-OSRM public) — calcul de l'itinéraire routier

   Paiement :
     • Stripe Elements (client) → api/create-payment-intent.js (Vercel)
...
// ── AUTOCOMPLETE (Base Adresse Nationale + Nominatim en complément) ────────
// La BAN (api-adresse.data.gouv.fr) est l'API officielle du gouvernement
...
function setupAC(inputId, listId, stateKey, biasKey) {
  ...
  const search = debounce(async (v) => {
    const bias = biasKey && booking[biasKey] ? { lat: booking[biasKey].lat, lon: booking[biasKey].lon } : null;
...
// ── INITIALISATION ─────────────────────────────────────────────────────────
setupAC('res-depart',  'list-depart',  'from', 'to');
setupAC('res-arrivee', 'list-arrivee', 'to',   'from');
